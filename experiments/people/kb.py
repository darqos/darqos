...
# A human person.
# WikiData Q5
person_item = add_item("human", desc="a human person")
# Needs a bunch of properties and qualifiers ...

start_time_qualifier = add_property("start time")
end_time_qualifier = add_property("end time")

# WikiData P735: given name
given_name_property = add_property("given name")
# optionally, should have date qualifiers for name changes

# WikiData P734: family name
family_name_property = add_property("family name")
# optionally, should have date qualifiers for name changes

# WikiData P1449: nickname
nick_name_property = add_property("nickname")
# optionally, should have date qualifiers for name changes

# I think there's no need to try to record gender or sex, so WikiData P21
# is probably irrelevant here.
preferred_pronouns_property = add_property("preferred pronouns")
# optionally, should have date qualifiers for changes

# WikiData P569. date of birth
date_of_birth_property = add_property("date of birth")

# WikiData P19: place of birth (city)
place_of_birth_property = add_property("place of birth")

#
date_of_death_property = add_property("date of death")

# WikiData P20: place of death (city)
place_of_death_property = add_property("place of death")

# WikiData P856: website (as part of vCard)
url_property = add_property("url")

# WikiData P968: email address (as part of vCard)
email_address_property = add_property("email address")

# WikiData P1329: phone number, no tel: prefix
phone_number_property = add_property("phone number", "P1329")
...
def test():
    ...
    s = Statement()
    s.claim.snak.property = "educated at"
    s.claim.snak.value = "St John's College"
    ...
    david.add_statement(find_property("spouse"),
                        louise.get_id(),
                        [Qualifier(find_property("start time"),
                                   "1998/11/14")])
    ...
    louise.add_statement(find_property("spouse"),
                         david.get_id(),
                         [Qualifier(find_property("start time"),
                                    "1998/11/14")])
    return
